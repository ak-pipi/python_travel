...
def test_travel_plan_generation():
    ...
    for i, summary in enumerate(test_summaries, 1):
        ...
        if result["status"] == "success":
            print("✅ 攻略生成成功!")
            print(f"⏱️  生成时间: {result.get('generation_time', 0)}秒")
            print(f"📊 字数: {result.get('word_count', 0)}")
            print(f"⭐ 质量评分: {result.get('quality_score', 0)}/10")
            print(f"🔍 使用搜索: {result.get('search_used', False)}")

            print("\n📝 攻略内容:")
            print(result["plan_text"][:500] + "..." if len(result["plan_text"]) > 500 else result["plan_text"])

            # 显示结构化信息
            structured = result.get("structured_plan", {})
